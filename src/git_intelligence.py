...
class GitIntelligence:
    ...
    def get_file_history(self, file_path: str, max_count: int = 10) -> Dict[str, Any]:
        """
    ...
    def get_line_blame(self, relative_file_path: str, start_line: int, end_line: int) -> List[Dict[str, Any]]:
        ...
        if not full_path.exists() or start_line <= 0 or end_line < start_line:
            return []
    ...
    def get_method_provenance(
        ...
        """
        Retrieves line-level commit provenance, targeted method history, and patch diffs.
        """
        clean_path_str = relative_file_path.replace("%2F", "/").replace("\\", "/")
        ...
        try:
            ...
            # Scoped line-range log command using robust RECORD_SEP delimiter
            record_sep = "<--COMMIT_END-->"
            try:
                log_raw = repo.git.log(
                    f"-n{max_commits}",
                    f"-L{start_line},{end_line}:{rel_path}",
                    f"--pretty=format:%H|%an|%ad|%s{record_sep}",
                    "--date=short"
                )
                
                raw_entries = [e.strip() for e in log_raw.split(record_sep) if e.strip()]
                
                for entry in raw_entries:
                    # Separate header line from optional diff body generated by -L
                    ...
                    if len(parts) == 4:
                        ...
                        # Extract diff produced by line-range log if present
                        diff_lines = lines[1:] if len(lines) > 1 else []
                        diff_output = "\n".join(diff_lines).strip()
                        
                        # Fallback to direct git show if line diff wasn't captured
                        if not diff_output:
                            try:
                ...
                # Fallback to standard file-level commits if -L range tracking fails
                fallback_history = self.get_file_history(clean_path_str, max_count=max_commits)
                for c in fallback_history.get("commits", []):
                    commits_list.append({
